...
class Config:
    ...
    PLOT_CHUNK_LEN = 120                   
    PLOT_PDF_PATH = "comprehensive_experiment_results.pdf" 
...
def build_model(input_shape):
    ...
    model.compile(optimizer=optimizers.Adam(learning_rate=0.001, clipnorm=1.0), loss=multi_quantile_loss)
    return model
...
if __name__ == "__main__":
    ...
    for m in modes:
        for t in range(cfg.TRIALS_PER_MODE):
            base_results.append(run_one_trial(cfg, m, t, use_weights=False))

    y_test_true = base_results[0]['y_te_true']
    test_rain_mask = base_results[0]['masks']['test_rain']

    # --- 新增：將 4 種 Base Model 的預測結果 (平均) 加入報表 ---
    for m in modes:
        # 抓出同一個特徵模式的 6 個模型預測值做平均
    ...
    print("\n" + "="*60 + f"\nPHASE 3: Rescue Mission (Rain Weight x{cfg.RAIN_WEIGHT})\n" + "="*60)
    rescue_preds = []
    ...
    all_metrics.extend(evaluate_all_conditions(y_test_true, y_rescue, test_rain_mask, "3. Rescue Model"))
    ...
    # 掃描測試集，找出含有最多雨天 (灰色遮罩) 的 120 小時區間
    for s in range(0, len(y_test_true) - cfg.PLOT_CHUNK_LEN, 24): # 每 24 小時滑動一次
    ...
    if max_rain_sum > 0:
        ...
        plot_time_series(y_test_true[best_s:best_e], y_global[best_s:best_e], test_rain_mask[best_s:best_e], 
                         title=f"Global Ensemble (Steps {best_s}-{best_e}) - Without Rescue", ax=ax1)
        plot_time_series(y_test_true[best_s:best_e], y_rescue[best_s:best_e], test_rain_mask[best_s:best_e], 
                         title=f"Rescue Model (Steps {best_s}-{best_e}) - Note the channels adapting to the grey rain areas!", ax=ax2)
        plt.tight_layout()
        print("\n>>> Pop-up window displaying! Please close the plot window to finish the program. <<<")
        plt.show() # <--- 這裡會彈出視窗讓你看圖
    else:
    ...
    with PdfPages(cfg.PLOT_PDF_PATH) as pdf:
        ...
        for s in range(0, n, chunk):
            ...
            plot_time_series(y_test_true[s:e], y_global[s:e], test_rain_mask[s:e], title=f"Global (Steps {s}-{e})", ax=ax1)
            plot_time_series(y_test_true[s:e], y_rescue[s:e], test_rain_mask[s:e], title=f"Rescue (Steps {s}-{e})", ax=ax2)
            plt.tight_layout()
            pdf.savefig(fig)
            plt.close(fig)
